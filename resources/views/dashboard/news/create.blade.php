...
@section('content')
    <div class="justify-center w-full">
        <div class="flex-col bg-white rounded-xl p-8 drop-shadow-lg">
            <h1 class="text-base text-gray-700 font-semibold mb-4">Tambah Kabar</h1>
            <hr />
            <br />
                ...
                <form action="{{ route('dashboard.news.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="flex flex-col mb-4 gap-2 w-4/12">
                            ...
                            <div id="image-preview-placeholder"
                                ...
                                <img id="preview-image" class="w-full h-[195px] object-cover rounded-2xl"
                                    src="{{ asset('images/news/placeholder.png') }}" alt="Image Placeholder">
                            </div>
                                ...
                                <select id="collaborator" class="w-full p-3 bg-white rounded-md border border-gray-200 placeholder-gray-500" name="created_by">
                                    ...
                                    @foreach ($collaborators as $collaborator)
                                        <option value="{{ $collaborator->user_id }}"
                                            {{ old('created_by') == $collaborator->user_id ? 'selected' : '' }}>
                                            {{ $collaborator->name }}
                                        </option>
                                    @endforeach
                                    <option value="{{ Auth::user()->id }}">Admin EntrepreneurHub</option>
                                </select>
                            </div>
                            @error('created_by')
                    ...
                    <div class="mb-4 mt-2">
                        ...
                        <textarea name="content" id="content"
                            class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500">{{ old('content') }}</textarea>
                        @error('content')
                    ...
                    <div class="mb-4 w-6/12 flex flex-col">
                        ...
                        @error('status')
                            <span class=" text-red-500 mt-2 text-sm">Mohon pilih status kabar terlebih dahulu!</span>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end mt-6 gap-2">
                        ...
                        <button type="submit" id="submitButton" class="btn btn-primary text-white font-semibold py-2 px-4 rounded">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
