...
<div class="justify-center max-xl:w-full">
    <div class="flex-col bg-white rounded-xl p-8 drop-shadow-lg">
        <h1 class="text-xl text-gray-800 mb-8">Edit Materi</h1>
        <form class="flex flex-col w-full" method="POST" enctype="multipart/form-data" action="{{ route('dashboard.templates.update', $template->id) }}">
            @csrf
            <div class="flex flex-col w-10/12 mb-4">
                ...
                <input type="text" name="title" placeholder="Judul Materi"
                    class="w-full py-2.5 px-3 bg-white rounded-md border border-gray-200 text-gray-800 placeholder-gray-500 focus:ring-primary" value="{{ old('title', $template->title) }}">
                @error('title')
            ...
            <div class="flex flex-col w-10/12 mb-4">
                ...
                <select name="tag_id[]" id="tag" multiple="multiple"
                    ...
                    @endphp
                    @foreach ($tags as $tag)
                        @foreach ($template->hasTags as $templateTag)
                            @if ($templateTag->id == $tag->id)
                                @php
                            ...
                            @endif
                        @endforeach
                        <option value="{{ $tag->id }}" {{ old('tag_id', $templateTagId) == $tag->id ? 'selected' : '' }}>{{ $tag->name ?? '' }}</option>
                    @endforeach
                </select>
                @error('tag_id')
            ...
            <div class="flex flex-col w-10/12 mb-4">
                ...
                <textarea id="description" name="description" rows="5" class="w-full bg-white rounded-md">{{ $template->description }}</textarea>
            </div>
            <div class="flex flex-col w-10/12 mb-4">
                <label class="text-gray-600 font-semibold mb-2">File Materi <span class="text-xs font-normal">(Max: 8MB)</span><span class="text-red-500">*</span><span class="text-red-500">*</span></label>
                <div class="w-full flex items-start gap-x-8">
                    ...
                    <div class="flex w-9/12">
                        <div class="relative flex items-center justify-center">
                            <p id="labelTemplate">{{ \Illuminate\Support\Str::afterLast($template->file, '/') }}</p>
                        </div>
                    </div>
                </div>
                @error('file')
            ...
            <div class="flex flex-col mb-6">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="status" id="status" value="1" class="sr-only peer" {{ old('status', $template->status) == true ? 'checked' : '' }}>
                    ...
                    <span id="labelStatus" class="ml-6 {{ $template->status ? 'text-green-600' : 'text-red-600'}}">{{ $template->status_label->value }}</span>
                </label>
            </div>
