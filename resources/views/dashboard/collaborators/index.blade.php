...
<div class="justify-center max-xl:w-full">
    <div class="flex-col bg-white rounded-xl p-8 drop-shadow-lg">
        <h1 class="text-xl text-gray-800 mb-6">Kolaborator</h1>
        <div class="flex flex-row items-center w-full mb-8">
            <div class="flex flex-row w-8/12">
                <form class="flex w-full" id="formSearch" method="GET" action="{{ route('dashboard.collaborators.index') }}">
            ...
            <div class="w-4/12 flex justify-end">
                <a href="{{ route('dashboard.collaborators.create') }}" class="px-6 py-3 bg-primary text-white rounded-xl">Tambah Kolaborator</a>
            </div>
        </div>
            ...
            <table class="border-collapse rounded-2xl min-w-full leading-normal">
                <thead class="rounded-2xl">
                    <tr class="rounded-2xl text-gray-500 font-normal text-md text-left whitespace-nowrap">
                        ...
                        <th class="py-2.5 px-4">Nama Kolaborator</th>
                        <th class="py-2.5 px-4">Direktur</th>
                ...
                <tbody class="rounded-2xl bg-white text-gray-700">
                    @forelse($collaborators as $key => $collaborator)
                        <tr>
                            <td class="py-2.5 px-4">
                                {{ $key + $collaborators->firstItem() }}
                            </td>
                            <td class="py-2.5 px-4">
                                <div class="flex flex-row items-center gap-6 px-2">
                                    <img class="w-16 object-fill rounded-2xl" src="{{ $collaborator->logo_url ?? asset('images/avatar-boy.png') }}" alt="{{ $collaborator->logo_url }}" />
                                    <p class="max-w-xs">{{ $collaborator->name }}</p>
                                </div>
                            </td>
                            <td class="py-2.5 px-4">
                                {{ $collaborator->director_name }}
                            </td>
                            <td class="py-2.5 px-4">
                                @forelse ($collaborator->hasTags as $collaboratorTag)
                                    <span class="mb-1">{{ $collaboratorTag->name }}</span><br>
                                @empty
                                    <span>-</span>
                                @endforelse
                            </td>
                            <td class="py-2.5 px-4">{{ $collaborator->hasCity ? ($collaborator->hasCity ? ($collaborator->hasCity->city_name) : '-') : '-' }}
                            </td>
                            <td class="py-2.5 px-4 w-1/12">
                                <span class="badge badge-{{ $collaborator->statusLabel->type }}">{{ $collaborator->statusLabel->value }}</span>
                            </td>
                            @role('admin')
                                <td class="py-2.5 px-4">
                                    <div class="flex flex-row items-center gap-x-4">
                                        <a href="{{ route('umkm.detail', [$collaborator->slug, 'type' => 'workshops']) }}" target="_blank"  title="Lihat"><ion-icon name="eye-outline" class="text-2xl text-primary"></ion-icon></a>
                                        <a href="{{ route('dashboard.collaborators.edit', $collaborator->id) }}" class="text-primary"  title="Ubah">
                                        ...
                                        {{-- <form method="POST" class="m-0 flex items-center form-delete-collaborator" action="{{ route('dashboard.collaborators.delete', $collaborator->id) }}">
                                            @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="p-5 text-center">Belum ada data Kolaborator yang dapat ditampilkan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if (!empty($collaborators))
            <div class="flex justify-end my-4">{{ $collaborators->links('vendor.pagination.custom') }}</div>
        @endif
    </div>
</div>
@endsection
...
<script>
    ...
    document.querySelectorAll('.form-delete-collaborator').forEach(form => {
