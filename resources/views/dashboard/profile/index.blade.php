    ...
    <div class="justify-center max-xl:w-full mb-8">
        <div class="flex-col bg-white rounded-xl p-8 drop-shadow-lg">
            <div class="flex justify-between">
                <span class="text-xl font-bold text-primary">Profile Saya</span>
                <a href="{{ route('dashboard.profile.edit') }}" type="button" class="btn btn-primary text-white font-semibold py-2 px-4 rounded cursor-pointer">Edit Profile</a>
            </div>
                    <form>
                    <div class="flex">
                        <div class="flex flex-col mb-4 gap-2">
                            <div class="text-lg font-semibold text-primary mt-3">Avatar</div>
                            <label for="avatar" class="text-xs block"></label>
                            <img class="w-[195px] h-[195px] object-cover rounded-2xl border-2 border-primary" src="{{ ($user->hasParticipant && $user->hasParticipant->hasBusiness) ? $user->hasParticipant->avatar_url == NULL ? asset('images/news/placeholder.png') : $user->hasParticipant->avatar_url : asset('images/news/placeholder.png') }}" alt="Avatar Placeholder">
                        </div>
                    ...
                    <div class="flex w-full gap-6">
                        <div class="flex flex-col w-6/12 gap-2">
                            <label for="state_code">Provinsi</label>
                            ...
                            <label for="sector_code">Kecamatan</label>
                        ...
                        <div class="flex flex-col w-6/12 gap-2">
                            <label for="city_code">Kabupaten</label>
                            ...
                            <label for="village_code">Kelurahan</label>
                    ...
                    <div class="flex flex-col w-full md:w-6/12 gap-2 my-4">
                        ...
                        <label for="nib_created_at">Tahub Pembuatan NIB</label>
                    ...
                    <div class="flex flex-col w-full md:w-6/12 gap-2 my-4">
                        ...
                        <div>
                            <input class="text-primary  focus:ring-primary h-4 w-4" id="platforms_shopee" type="checkbox" name="platforms" value="shopee" {{ ($user->hasParticipant && $user->hasParticipant->hasBusiness) ? isExistPlatform($user->hasParticipant->hasBusiness->platforms,  'shopee') ? 'checked' : '' : '' }} disabled>
                            <label for="platforms_shopee" class="ml-2">Shopee</label>
                        </div>
                        <div>
                            <input class="text-primary focus:ring-primary h-4 w-4" id="platforms_tokopedia" type="checkbox" name="platforms" value="tokopedia" {{ ($user->hasParticipant && $user->hasParticipant->hasBusiness) ? isExistPlatform($user->hasParticipant->hasBusiness->platforms,  'tokopedia') ? 'checked' : '' : '' }} disabled>
                            <label for="platforms_tokopedia" class="ml-2">Tokopedia</label>
                        </div>
                        <div>
                            <input class="text-primary  focus:ring-primary h-4 w-4" id="platforms_lazada" type="checkbox" name="platforms" value="lazada" {{ ($user->hasParticipant && $user->hasParticipant->hasBusiness) ? isExistPlatform($user->hasParticipant->hasBusiness->platforms,  'lazada') ? 'checked' : '' : ''}} disabled>
                            <label for="platforms_lazada" class="ml-2">Lazada</label>
                        </div>
                        <div>
                            <input class="text-primary  focus:ring-primary h-4 w-4" id="platforms_bukalapak" type="checkbox" name="platforms" value="bukalapak" {{ ($user->hasParticipant && $user->hasParticipant->hasBusiness) ? isExistPlatform($user->hasParticipant->hasBusiness->platforms,  'bukalapak') ? 'checked' : '' : '' }} disabled>
