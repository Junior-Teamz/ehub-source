...
<div class="justify-center max-xl:w-full">
    <div class="flex-col bg-white rounded-xl p-8 drop-shadow-lg">
        <h1 class="text-xl text-gray-800 mb-6">Mentor</h1>
        <div class="flex flex-row items-center w-full mb-8">
            <div class="flex flex-row w-8/12">
                <form class="flex w-full" id="formSearch" method="GET" action="{{ route('dashboard.mentors.index') }}">
            ...
            <div class="w-4/12 flex justify-end">
                <a href="{{ route('dashboard.mentors.create') }}" class="px-6 py-3 bg-primary text-white rounded-xl">Tambah Mentor</a>
            </div>
        </div>
            ...
            <table class="border-collapse rounded-2xl min-w-full leading-normal">
                <thead class="rounded-2xl">
                    <tr class="rounded-2xl text-gray-500 font-normal text-md text-left whitespace-nowrap">
                        ...
                        <th class="py-2.5 px-4">Nama Mentor</th>
                        <th class="py-2.5 px-4">Kolaborator</th>
                        <th class="py-2.5 px-4">Profesi / Jabatan / Keahlian</th>
                        <th class="py-2.5 px-4">Bidang</th>
                ...
                <tbody class="rounded-2xl bg-white text-gray-700">
                    @forelse($mentors as $key => $mentor)
                        <tr>
                            <td class="py-2.5 px-4">
                                {{ $key + $mentors->firstItem() }}
                            </td>
                            <td class="py-2.5 px-4">
                                <div class="flex flex-row items-center gap-6 px-2">
                                    <img class="w-16 object-fill rounded-2xl" src="{{ $mentor->avatar_url ?? asset('images/avatar-boy.png') }}" alt="{{ $mentor->fullname }}" />
                                    <p class="max-w-xs">{{ $mentor->fullname }}</p>
                                </div>
                            </td>
                            <td class="py-2.5 px-4">
                                {{ $mentor->hasCollaborator ? $mentor->hasCollaborator->name : '-' }}
                            </td>
                            <td class="py-2.5 px-4">
                                {{ $mentor->expertise }}
                            </td>
                            <td class="py-2.5 px-4">
                                @forelse ($mentor->hasExperts as $mentorExpert)
                                    <span class="mb-1">{{ $mentorExpert->name }} {{ $loop->last ? '' : ','}}</span><br>
                                @empty
                                    <span>-</span>
                                @endforelse
                            </td>
                            <td class="py-2.5 px-4 w-1/12">
                                <span class="badge badge-{{ $mentor->statusLabel->type }}">{{ $mentor->statusLabel->value }}</span>
                            </td>
                            @hasanyrole('admin|collaborator|institution')
                                <td class="py-2.5 px-4">
                                    <div class="flex flex-row items-center gap-x-4">
                                        ...
                                        <a href="{{ route('dashboard.mentors.edit', $mentor->id) }}" class="text-primary" title="Ubah">
                                        ...
                                        <form method="POST" class="m-0 flex items-center form-delete-mentor" action="{{ route('dashboard.mentors.delete', $mentor->id) }}">
                                            @csrf
        ...
        @if (!empty($mentors))
            <div class="flex justify-end my-4">{{ $mentors->links('vendor.pagination.custom') }}</div>
        @endif
    </div>
</div>
@endsection
